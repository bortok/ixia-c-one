...
func TestBGPRouteInstall(t *testing.T) {
	...
	config, expected := bgpRouteInstallConfig(client)
	...
	if _, err := dut.PushDutConfigFile("configs/bgp_route_install/set_dut.txt"); err != nil {
		t.Fatal(err)
	}
	defer dut.PushDutConfigFile("configs/bgp_route_install/unset_dut.txt")
...
func bgpRouteInstallConfig(client *helpers.ApiClient) (gosnappi.Config, helpers.ExpectedState) {
